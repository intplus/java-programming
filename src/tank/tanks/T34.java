    ...
    public Action setUp() throws Exception {
        ...
        switch (choice) {
            case 0:
                ...
                PrintToFile.update("t34.txt", "MOVE " + this.getDirection());
                break;
            case 1:

                act = 1;
                PrintToFile.update("t34.txt", "FIRE");
                break;
        }
...
//    public Action setUp() throws Exception {
//
//        ArrayList list = RepeatTanks.readFile("t34.txt");
//        int act = 0;
//        System.out.println("T34 = " + list.get(i));
//        Thread.sleep(1000);
//
...
//    }
    ...
    private boolean lineScannerEagle() {
        for (int i = 1; i < 9; ++i) {
            if (checkBrick(this.getY()/64, this.getX()/64 - i)) {
                break;
            }
            if (checkBrick(this.getY()/64, this.getX()/64 + i)) {
                break;
            }
            if (checkBrick(this.getY()/64 + i, this.getX()/64)) {
                break;
            }
            if (checkEagle(this.getY()/64, this.getX()/64 - i)) {
                System.err.println("eagle");
                return true;
            }
            if (checkEagle(this.getY()/64, this.getX()/64 + i)) {
                System.err.println("eagle");
            ...
            if (checkEagle(this.getY()/64 + i, this.getX()/64)) {
                System.err.println("eagle");
                return true;
            }
